subroutine rkf45b(neqn,t0,tf,u0,nsteps,abserr,relerr,u)
! 
! Subroutine rkf45b computes an ODE solution by a variable
! step RK Fehlberg method for a series of points along the
! solution by repeatedly calling subroutine ssrkf45 for a
! single RK Fehlberg step. The truncation error is
...
do i=1,neqn
    if(dabs(e(i)).gt.(dabs(u(i))*relerr+abserr)/32.0d0)then
    ! 
