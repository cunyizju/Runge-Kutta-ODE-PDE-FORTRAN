subroutine rkc4b(neqn,t0,tf,u0,nsteps,abserr,relerr,u)
! 
! Subroutine rkc4b computes an ODE solution by a variable
! step classical fourth order RK method for a series of
! points along the solution by repeatedly calling
! subroutine ssrkc4 for a single classical fourth order RK
! step. The truncation error is estimated along the
! solution to adjust the integration step according to a
...
do i=1,neqn
if(dabs(e(i)).gt.(dabs(u(i))*relerr+abserr)/16.0d0)then
! 
